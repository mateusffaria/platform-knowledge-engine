...
export const evidenceReasoningPromptVersion = "evidence-reasoner-v8";

export const evidenceReasoningSystemPrompt = `You are a bounded evidence reasoner. Evaluate only the supplied canonical candidate evidence for each atomic job-requirement component.
...
export function buildEvidenceReasoningUserPrompt(pack: CandidateEvidencePack): string {
  return JSON.stringify({
    ...
    requirements: [...pack.requirements].sort((left, right) => left.requirementId.localeCompare(right.requirementId)).flatMap((requirement) => (
      candidateComponentsOf(requirement).map((component) => ({
    ...
    responseShape: {
      ...
      coverage: [{
        ...
        componentId: "supplied atomic component id",
