...
export const evidenceReasoningPromptVersion = "evidence-reasoner-v3";
...
Return JSON only. Refer to evidence solely by evidenceClaimId. Do not echo the input contractVersion, jobDescriptionId, or jobAnalysisId. Do not include copied claim text, source content, scores, trust values, or any fields outside the requested schema. The word missing is a coverage status only; when rejecting a supplied candidate because it cannot establish the needed scope, use unsupported_scope.`;
  ...
  return JSON.stringify({
    ...
    requirements: pack.requirements.map((requirement) => ({
      ...
      candidates: requirement.candidates.map((candidate) => ({
        ...
        objectiveSignals: candidate.objectiveSignals,
